...
namespace
{
  ...
  void trace(std::stack<GroceryItem> const &sourceCart, std::stack<GroceryItem> const &destinationCart, std::stack<GroceryItem> const &spareCart, std::ostream &s = std::clog)
  {
    ...
    for (auto &&[key, currentCart] : groceryCarts)
      s << std::setw(25) << currentCart.label;                                                          // print the column labels
    ...
    for (; tallestStackSize > 0; --tallestStackSize) // for each grocery item in a cart
    {
      ...
      for (auto &&[key, currentCart] : groceryCarts) // for each grocery item cart
      {
        if (currentCart.cart.size() == tallestStackSize) // if the current cart is this tall, print the top grocery item
        {
          ...
          if (name.size() > 23)
            name[21] = name[22] = name[23] = '.'; // replace last few characters of long names with "..."
          s << std::left << std::setw(25) << name.substr(0, 23) << std::right;
          currentCart.cart.pop();
        }
        else
          s << std::string(25, ' '); // otherwise, nothing to print in this cart so print whitespace instead
      }
      s << '\n';
    }
  ...
  *********************************************************************************************************************************/
  ...
  void carefully_move_grocery_items(std::stack<GroceryItem> &from, std::stack<GroceryItem> &to)
  {
    ...
    std::stack<GroceryItem> link;
    trace(from, to, link);
    carefully_move_grocery_items(from.size(), from, to, link);
    /////////////////////// END-TO-DO (2) ////////////////////////////
  }
} // namespace
...
int main(int argc, char *argv[])
{
  ...
  myCart.push(GroceryItem("Garelick Farms Dairy Pure Milk 1% Lowfat", "Garelick Farms", "00075457129000", 9.64));
  myCart.push(GroceryItem("Kellogg's Cocoa Krispies Cereal", "Rice Krispies", "38000475443", 5.67));
  myCart.push(GroceryItem("Hotdog Uncrd Big Apple", "Applegate Farms", "00025317533003", 12.57));
  myCart.push(GroceryItem("Apple Pie", "Apple Pie", "09073649000493", 13.99));
  myCart.push(GroceryItem("Fiber One Bread Country White", "Fiber One", "00835841005255", 3.87));
  myCart.push(GroceryItem("Nature's Promise Naturals Fresh Brown Eggs Omega 3 Large", "Nature's Promise", "00688267039317", 24.66));
  /////////////////////// END-TO-DO (4) ////////////////////////////
  ...
  GroceryItem *upcCheck;

  while (!checkoutCounter.empty())
  {

    upcCheck = worldWideDatabase.find(checkoutCounter.front().upcCode());

    if (upcCheck == nullptr)
    {
      std::cout << checkoutCounter.front() << " UPC does not exist, item will have no charge." << std::endl;
    }
    else
    {
      std::cout << checkoutCounter.front().upcCode() << "\n";
      amountDue += upcCheck->price();
    }
    checkoutCounter.pop();
  }
  /////////////////////// END-TO-DO (7) ////////////////////////////
  ...
  if (std::abs(amountDue - expectedAmountDue) < 1E-4)
